...
from fastretrieval.models.image.transform.functional import (
    center_crop,
    convert_to_rgb,
    crop_ndarray,
    normalize,
...
class Compose:
    ...
    def _get_center_crop(transforms: list[Transform], config: dict[str, Any]) -> None:
        ...
        if mode in ("CLIPImageProcessor", "SiglipImageProcessor"):
            ...
            or mode == "JinaCLIPImageProcessor"
            or mode == "Idefics3ImageProcessor"
        ):
            pass
        else:
            raise ValueError(f"Preprocessor {mode} is not supported")
    ...
    def _get_pad2square(transforms: list[Transform], config: dict[str, Any]) -> None:
        mode = config.get("image_processor_type", "CLIPImageProcessor")
        if mode == "CLIPImageProcessor" or mode == "ConvNextFeatureExtractor":
            pass
        elif mode == "JinaCLIPImageProcessor":
            transforms.append(
                PadtoSquare(
